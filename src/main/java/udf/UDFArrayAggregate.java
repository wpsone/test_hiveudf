...
@Description(name = "array_aggregate",
    value = "_FUNC_(array,function) - Aggregate elements of the array using the function in the UDAF")
public class UDFArrayAggregate extends GenericUDF {
    ...
    public Object evaluate(DeferredObject[] deferredObjects) throws HiveException {
        List<?> inputs = (List<?>)ObjectInspectorUtils.copyToStandardObject(deferredObjects[0].get(),listOI);
        if (inputs == null){
            return null;
        }
        if (evaluator == null){
            Text functionName = (Text)ObjectInspectorUtils.copyToStandardObject(deferredObjects[1].get(),stringOI);
            if (functionName == null){
                throw new HiveException("Function name cannot be null");
            }

            GenericUDAFResolver resolver = FunctionRegistry.getGenericUDAFResolver(functionName.toString());
            if (resolver == null){
                throw new HiveException("Could not find function with name"+functionName.toString());
            }
            ...
            typeInfoArray[0] = TypeInfoUtils.getTypeInfoFromObjectInspector(elementOI);
