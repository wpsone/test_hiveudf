...
@Description(name = "udfgreatcircle",
...
public class UDFGreatCircleDist extends UDF {
    public double evaluate(Double lat1,Double long1,Double lat2,Double long2) {
        ...
        double t2 = Math.cos(lat1)*Math.sin(lat2)-Math.sin(lat1)*Math.cos(lat2);
        double t3 = Math.sin(lat1)*Math.sin(lat2)+Math.cos(lat1)*Math.cos(lat2);
